...
st.set_page_config(
    page_title="ELISA Data Analysis Pipeline",
    page_icon="🧪",
    layout="wide"
)
...
def expand_well_selection(selection_str, df_index, df_columns):
    ...
    for t in tokens:
        ...
        if len(t) >= 2 and t[0] in rows and t[1:] in cols:
            if t not in expanded_wells:
                expanded_wells.append(t)
...
def process_blank_deduction(df_raw, apply_blank, blank_str):
    ...
    if not valid_blanks or not apply_blank:
        return df_raw.copy(), 0.0, [], None
...
def parse_sample_cols(cols_str):
    ...
    for t in tokens:
        ...
        if ':' in t or '-' in t:
            ...
            if t not in parsed_cols:
                parsed_cols.append(t)
...
st.header("📂 STEP 1: Multi-Wavelength Raw Data Upload")
...
with st.expander("🔍 파싱된 Plate OD Map 확인", expanded=False):
    view_mode = st.radio(
        "표시 옵션",
        options=['450-650nm Reduced OD', '450nm Raw OD', '650nm Ref OD'],
        horizontal=True
    )
    if 'Reduced' in view_mode and df_reduced is not None:
        st.dataframe(df_reduced.style.format("{:.4f}").background_gradient(cmap="Greens"))
    elif '450nm Raw' in view_mode and df_450 is not None:
        st.dataframe(df_450.style.format("{:.4f}").background_gradient(cmap="Blues"))
    elif '650nm Ref' in view_mode and df_650 is not None:
        st.dataframe(df_650.style.format("{:.4f}").background_gradient(cmap="Purples"))

st.markdown("---")

# STEP 2: Blank & QC Analysis
st.header("🧪 STEP 2: Blank & QC Analysis Module")
...
with col2:
    qc_base_option = st.radio(
        "QC Target OD 계산 기준",
        options=['Blank 차감 후 최종 OD 기준', '450-650nm 원본 OD 기준 (Blank 차감 전)'],
        index=0
    )
...
df_final_od, blank_val, active_blanks, blank_summary_df = process_blank_deduction(
    df_plate, use_blank, blank_wells_str
)
...
qc_summary_df, active_qc_wells, target_od = calculate_qc_metrics(
    df_qc_source, qc_wells_str, divisor=divisor, source_tag=tag
)
...
# STEP 3: 4PL & InterpX Pipeline
st.header("📈 STEP 3: 4PL Regression & InterpX Titer Analysis")
...
with d_col2:
    base_start = st.number_input("일괄 시작 희석비", value=300.0, step=10.0)

with d_col3:
    base_fold = st.number_input("일괄 Fold 배수", value=3.0, step=0.5)

with d_col4:
    st.write(" ")
    st.write(" ")
    if st.button("전체 열 희석 일괄 적용"):
        for c in range(1, 13):
            st.session_state.col_dilution_config[str(c)] = {'start': base_start, 'fold': base_fold}
        st.success("희석비 조건 일괄 설정 완료!")

with st.expander("🛠️ 개별 열 희석 조건 변경 (선택 사항)"):
    sub_c1, sub_c2, sub_c3 = st.columns([1, 1, 1])
    with sub_c1:
        sel_col = st.selectbox("수정할 샘플 열", options=[str(i) for i in range(1, 13)])
    with sub_c2:
        mod_start = st.number_input("시작 희석비", value=st.session_state.col_dilution_config[sel_col]['start'], key=f"start_{sel_col}")
    with sub_c3:
        mod_fold = st.number_input("Fold 배수", value=st.session_state.col_dilution_config[sel_col]['fold'], key=f"fold_{sel_col}")

    st.session_state.col_dilution_config[sel_col] = {'start': mod_start, 'fold': mod_fold}
...
for col in sample_cols:
    ...
    if popt is not None:
        ...
        fit_results[col] = {
            'x': x_series, 'y': y_series, 'popt': popt, 'r2': r2, 'interp_x': calc_x,
            'start': s_dil, 'fold': f_val
        }
        summary_list.append({
            'Column': f"Column {col}",
            'Start Dilution': s_dil,
            'Fold Factor': f_val,
            'Bottom (A)': round(A, 6),
            'HillSlope (B)': round(B, 6),
            'EC50 (C)': round(C, 6),
            'Top (D)': round(D, 6),
            'R2': round(r2, 5),
            f'InterpX Titer ({target_od:.4f} OD)': round(calc_x, 2) if not np.isnan(calc_x) else "NaN"
        })
    else:
        summary_list.append({
...
st.header("💾 STEP 4: Export Analysis Reports")
